...
METHOD_COLORS = {
    ...
    'pAM':'#00ff9f','HealNet':'#ff9900','SMILe':'#ffe66d',
}
METHOD_DISPLAY = {
    ...
    'pAM':'pAM','HealNet':'HealNet','SMILe':'SMILe',
}
...
def load_all(analysis_dir: Path) -> tuple:
...
def main():
    ...
    print(f'[M3TRICS Dashboard] data dir: {adir.resolve()}')
    data, avail_modes, avail_modalities, cohort_endpoints = load_all(adir)
    if not data:
...
/* ── LAYOUT ──────────────────────────────── */
.app{display:flex;height:100vh;background:var(--bg);color:var(--t1)}
...
.pvbtn{position:relative;font-size:12px;font-weight:700;padding:5px 12px;border-radius:20px;
...
.dd-btn,.dd-item,.cb,.ni,.pvbtn,.mf-btn,.mf-item,.tpill,.cg-cell,.mt th,.mt tr{
...
.ch{padding:10px 14px 9px;border-bottom:1px solid var(--bd);
...
.traj-widget-board.traj-side .pvbtn{padding:5px 9px;font-size:11.5px}
...
.mt th{padding:8px 11px;text-align:left;font-size:12px;font-weight:700;
...
.mt td{padding:8px 11px;color:var(--t2);border-bottom:1px solid var(--bd);white-space:nowrap}
...
<aside class="sb">
  ...
  <nav class="sb-nav">
    ...
    <div class="sec-lbl" style="margin-top:12px">Static-Cohort</div>
      ...
      <div class="traj-widget-board traj-stack" id="traj-widgets">
        <div class="traj-slot" data-widget="perf">
          <div class="card traj-card" data-widget="perf" data-plot="ch-tr">
            ...
            <div class="traj-resize-handle" data-plot="ch-tr" title="Drag to resize trajectory panel"></div>
          </div>
        </div>
        <div class="traj-slot" data-widget="deg">
          <div class="card traj-card" data-widget="deg" data-plot="ch-dg">
            <div class="ch traj-drag-handle"><span class="ct2">Method-level Degradation Trajectories &mdash; ratio = Baseline / Performance<sub>i</sub></span>
              <div class="pv-btns">
            ...
            <div class="traj-resize-handle" data-plot="ch-dg" title="Drag to resize trajectory panel"></div>
          </div>
        </div>
      </div>
    </div>
...
const anyDropdownOpen = () => !!document.querySelector('.dd-menu.open,.mf-menu.open');
...
function attachTrajectoryTooltip(gd,kind){
  ...
  gd._m3TipHover=function(ev){
    ...
    const px=rect.left+(ax&&ax.l2p?ax._offset+ax.l2p(pt.x):(e.clientX||rect.left)-rect.left);
    const py=rect.top +(ay&&ay.l2p?ay._offset+ay.l2p(pt.y):(e.clientY||rect.top)-rect.top);
    let flip=false;
    let x=px+14;
    let y=py-h/2;
...
function attachTrajectoryLegendEmpty(gd,msg){
  ...
  gd._m3EmptyHandler=()=>setTimeout(()=>syncTrajectoryLegendEmpty(gd,msg),0);
...
const TRAJ_MARGIN = {t:86,b:72,l:68,r:32};
const TRAJ_LEGEND = {orientation:'h',x:0.5,xanchor:'center',y:1.18,yanchor:'bottom',font:{size:13.5},bgcolor:'rgba(0,0,0,0)',groupclick:'togglegroup'};
const TRAJ_LEGEND_OFFSET_PX = (TRAJ_LEGEND.y - 1) * (TRAJ_HEIGHT - TRAJ_MARGIN.t - TRAJ_MARGIN.b);
function trajectoryLegendY(h){return 1 + TRAJ_LEGEND_OFFSET_PX / Math.max(160,h - TRAJ_MARGIN.t - TRAJ_MARGIN.b)}
function trajectoryLegend(h){return {...TRAJ_LEGEND,y:trajectoryLegendY(h)}}
...
function applyTrajectoryHeight(divId,h){
  ...
  TRAJ_HEIGHT_RAF=requestAnimationFrame(()=>{
    ...
    entries.forEach(([id,height])=>{
      ...
      if(Math.abs((gd._m3LastHeight||0)-height)<1)return;
      gd._m3LastHeight=height;
      Plotly.relayout(gd,{height,'legend.y':trajectoryLegendY(height)});
      if(gd._m3FullLayoutForEmpty){
        gd._m3FullLayoutForEmpty.height=height;
        gd._m3FullLayoutForEmpty.legend=trajectoryLegend(height);
      }
    });
  });
}
...
function rebuildCohorts(study){
  ...
  cbEl.innerHTML='';
  if(study==='static'){
    const dks=Object.keys(STATIC_DS_DATA);
    if(!dks.length){
      cbEl.innerHTML='<div style="font-size:10px;color:var(--t3);padding:8px 10px">No datasets found.</div>';
      return;
    }
    dks.forEach((k,i)=>{
      const m=(STATIC_DS_DATA[k]||{}).meta||{};
      const b=document.createElement('button');
      b.className='cb'+(k===S.ds?' on':''); b.dataset.k=k;
      b.onclick=()=>switchDs(k);
      b.innerHTML=`<span class="cdot" style="background:${dcs[i%3]}"></span><span>${m.name||k}</span>`;
      cbEl.appendChild(b);
    });
    if(!dks.includes(S.ds)){S.ds=dks[0]||null;}
    updateEndpointDropdown();
  } else {
    // Progressive: one button per cohort (grouped)
    ...
    // Ensure S.ds is valid in current mode/modality
    if(!dks.has(S.ds)){
...
function nav(s, study='progressive'){
  ...
  const hasData = study==='static'
    ? Object.keys(STATIC_DS_DATA).length > 0
...
function setTrajectoryWidgetLayout(layout,order=null){
  ...
  if(layout==='side'){
    ...
    setTrajectoryHeight('ch-tr',h);
    setTrajectoryHeight('ch-dg',h);
  }
...
function showTrajectoryDropPreview(board,card,widget,mode,dx,dy){
  ...
  const draggedH=card.getBoundingClientRect().height;
  const otherH=otherCard?otherCard.getBoundingClientRect().height:draggedH;
  const bw=board.clientWidth;
  const sideW=(bw-gap)/2;
  let left=0,top=0,width=bw,height=draggedH;
  if(mode==='side'){
    width=sideW;height=Math.max(draggedH,otherH);top=0;left=dx<0?0:sideW+gap;
  }else{
  ...
  board.querySelectorAll('.traj-drag-handle').forEach(handle=>{
    handle.addEventListener('pointerdown',ev=>{
      ...
      const move=e=>{
        ...
        card.style.transform=`translate3d(${dx*.18}px,${dy*.18}px,0) scale(1.012)`;
...
function positionMethodMenu(){
  ...
  const left=Math.min(Math.max(8,center-menuW/2),window.innerWidth-menuW-8);
  const top=Math.min(r.bottom+7,window.innerHeight-8);
...
function rStaticPerf(){
  ...
  const pv=f?.p_value; const sig=f?.significant_p0_05;
...
// ── GLOBAL ────────────────────────────────────────────────────────────────
function rGlobal(){rCards();renderMethodFilter();syncTrajectoryWidgets();rHeatmaps();rTraj();rDeg();}
...
function rCards(){
  ...
  if(f){
    ...
    document.getElementById('s-fs').textContent=f.significant_p0_05?'✓ Significant (p<0.05)':'Not significant';
  }
...
function rHeatmaps(){
  ...
  if(scaleIdx>=0){
    const axN=scaleIdx===0?'':scaleIdx+1;
    const dom=(layout[`yaxis${axN}`]||{}).domain||[0,1];
    traces[scaleIdx].colorbar={...traces[scaleIdx].colorbar,
      y:(dom[0]+dom[1])/2,
      len:Math.max(0.01,dom[1]-dom[0]),
      yanchor:'middle'};
  }
...
function rTraj(){
  ...
  const layout={...th,
    grid:{rows:1,columns:vp.length,pattern:'independent',xgap:.12},height:trajectoryHeight('ch-tr'),
    margin:TRAJ_MARGIN,showlegend:true,
    legend:trajectoryLegend(trajectoryHeight('ch-tr')),
    annotations,
  };
...
function rDeg(){
  ...
  const layout={...th,
    grid:{rows:1,columns:vp.length,pattern:'independent',xgap:.12},height:trajectoryHeight('ch-dg'),
    margin:TRAJ_MARGIN,showlegend:true,
    legend:trajectoryLegend(trajectoryHeight('ch-dg')),
    annotations,
  };
  const domains=axisDomains(vp.length);
  vp.forEach((p,i)=>{
